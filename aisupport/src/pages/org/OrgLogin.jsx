    ...
    <AuthShell title="Organization Login" subtitle="Access Support Agent, Team Manager, and Business Executive workspaces.">
      ...
      <form onSubmit={submit} className="space-y-4">
        <label className="block text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Work Email
          <div className="relative mt-2"><Mail size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-cyan-300" /><input className="w-full px-10 py-3.5 text-sm" placeholder="name@enterprise.com" type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div>
        </label>
        <label className="block text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Password
          <div className="relative mt-2">
            ...
            <input className="w-full px-10 py-3.5 pr-12 text-sm" placeholder="Enter password" type={showPassword ? 'text' : 'password'} required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
        ...
        <GlowButton className="w-full" disabled={loading} icon={Building2}>{loading ? 'Signing in...' : 'Sign in to Organization'}</GlowButton>
      </form>
