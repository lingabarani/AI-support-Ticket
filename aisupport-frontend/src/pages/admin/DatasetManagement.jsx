...
import { Database, Download, Eye, FileJson, FileSpreadsheet, RefreshCw, Trash2, UploadCloud } from 'lucide-react';
...
export default function DatasetManagement() {
  ...
  const acceptFile = (selected) => {
    ...
    if (selected.size > 10 * 1024 * 1024) {
      setToast('Maximum file size is 10 MB.');
      return;
    }
    setFile(selected);
    setSummary(null);
  ...
  const upload = async () => {
    if (!file) {
      setToast('Choose a CSV or JSON file first.');
      return;
    }
  ...
  const preview = async (uploadId) => {
    try {
      ...
      setToast(error.message || 'Preview failed.');
    }
  };
      ...
      <div className="slide-in space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
          ...
          <section className="card-glass rounded-xl p-5">
            ...
            {summary ? (
              ...
              <div className="rounded-xl border border-white/10 p-5 text-sm text-slate-400" style={{ background: 'rgba(255,255,255,0.03)' }}>
                Upload a dataset to view validation and insert summary.
              </div>
            )}
        ...
        <section className="card-glass rounded-xl p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button type="button" onClick={loadUploads} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-300 hover:bg-white/10">
              Refresh
            </button>
          </div>
          <div className="overflow-x-auto">
            <table>
              <thead>
                <tr>
                  <th>File</th><th>Type</th><th>Rows</th><th>Failed</th><th>Status</th><th>Uploaded</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {uploads.length ? uploads.map((upload) => (
                  <tr key={upload.uploadId}>
                    <td className="text-slate-300">{upload.fileName}</td>
                    <td className="text-slate-400">{upload.datasetType}</td>
                    <td className="text-slate-400">{upload.successRows}/{upload.totalRows}</td>
                    <td className="text-slate-400">{upload.failedRows}</td>
                    <td className="text-slate-400">{upload.status}</td>
                    <td className="text-slate-500 text-xs">{formatDate(upload.createdAt)}</td>
                    <td>
                      <div className="flex gap-2">
                        <button type="button" onClick={() => preview(upload.uploadId)} className="rounded-lg p-2 text-blue-300 hover:bg-white/10" title="Preview"><Eye size={15} /></button>
                        <button type="button" onClick={refreshDashboards} className="rounded-lg p-2 text-emerald-300 hover:bg-white/10" title="Refresh dashboards"><Download size={15} /></button>
                        <button type="button" onClick={() => remove(upload.uploadId)} className="rounded-lg p-2 text-red-300 hover:bg-white/10" title="Delete upload"><Trash2 size={15} /></button>
                      </div>
                    </td>
                  </tr>
                )) : (
