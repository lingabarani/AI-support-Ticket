...
const validateAndNormalizeTicketRow = (row, index, uploadId) => {
  const ticketDescription = asText(row.ticket_description || row.description || row.subject || row.issue_description);
  ...
  if (!asText(row.customer_email || row.email) && !asText(row.customer_name)) {
  ...
  return {
    ...
    ticket: {
      ...
      customer_email: asText(row.customer_email || row.email),
